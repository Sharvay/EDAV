...
# Load the trained model and datasets for visualizations and predictions
...
# ********************************************************************************Application UI***************************************************************************************************
# Defining the UI for our application
ui <- navbarPage(
  theme = shinytheme("flatly"), 
  "Diabetes Prediction & Analysis Tool",
  
  # First Tab: Prediction Inputs and Feature Importance
  tabPanel(
    ...
    fluidRow(
      column(12, plotOutput("feature_importance"))
    )
  ),
  
  # Second Tab: Data Analysis Visualizations
  tabPanel(
    "Visualizations",
    fluidRow(
  ...
  observeEvent(input$predict, {
    ...
    if (nrow(input_data) > 0 && all(!is.na(input_data))) {
      ...
      output$prediction <- renderUI({
        HTML(paste("<b style='color: #333333; font-size: 18px;'>Diabetes Prediction:</b> <span style='font-weight: bold; color: #333333;'>", interpretation, "</span>"))
      })
